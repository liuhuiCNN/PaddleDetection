...
class S2ANetBBoxPostProcess(object):
    ...
    def post_process(self, bboxes, bbox_num, im_shape, scale_factor):
        ...
        """
        print('bboxes xxxx', bboxes.shape)
        origin_shape = paddle.floor(im_shape / scale_factor + 0.5)

        origin_h = origin_shape[0]
        origin_w = origin_shape[1]

        bboxes[:, 0] = bboxes[:, 0] / scale_factor[0]
        bboxes[:, 1] = bboxes[:, 1] / scale_factor[1]
        bboxes[:, 2] = bboxes[:, 2] / scale_factor[0]
        bboxes[:, 3] = bboxes[:, 3] / scale_factor[1]
        bboxes[:, 4] = bboxes[:, 4] / scale_factor[0]
        bboxes[:, 5] = bboxes[:, 5] / scale_factor[1]
        bboxes[:, 6] = bboxes[:, 6] / scale_factor[0]
        bboxes[:, 7] = bboxes[:, 7] / scale_factor[1]
